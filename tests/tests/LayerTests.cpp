...
bool LayerTests::TestCNNMultiple()
{
    ...
    for (size_t i = 0; i < 18; i++) {
        if (std::abs(res->data[i] - correct_res_cnn1[i]) > 1e-6) {
            res->Print(0);
            res->Print(1);
    ...
    for (int i = 0; i < 6; i++) {
        if (std::abs((*delta)[i] - correct_delta_cnn2[i]) > 1e-6) {
            ...
            std::cout << "Second layer delta output mismatch at index " << i
                      ...
                      << ", got " << (*delta)[i] << std::endl;
            return false;
        }
    }

    for (int i = 0; i < 3; i++) {
        if (std::abs((*delta_bias)[i] - correct_delta_bias_cnn2[i]) > 1e-6) {
            delta_bias->Print(i);
